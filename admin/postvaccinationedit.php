...
$vac_id=$_GET['vac_id'];
 $query = " select * from tbl_vaccination where vac_id='$vac_id'";
...
<tr>
<td>Title</td>
<td><input type="text" name="title" value="<?php echo $r1['vac_age']?>" class="form-control" required=""/></td>
</tr>
<tr>
<td>Description</td>
<td><textarea name="des" value="" class="form-control" required=""><?php echo $r1['vac_des']?></textarea></td>
</tr>
<tr>
<!-- <td>Image</td>
<td>
    <img src="../uploads/<?php echo $r1['vac_img']?>" width="80" height="80"/>
...
<td></td>
<td>
    <!-- <input type="hidden" name="old" value="<?php echo $r1['vac_img']?>"/> -->
...
if($file_name!='')
{
    ...
    $pic=$old;
             ...
             if( mysqli_query($con,"update tbl_vaccination set vac_age='$title',vac_des='$des' where vac_id='$vac_id'"))
                {
                    echo '<p style="color:green;font-size:15px;font-weight:bold;
                    margin-left:100px">Details Saved Successfully.</p>';
                    echo '<script>window.location.replace("postvaccination.php")</script>';
                  }else{
