...
$babycare_id=$_GET['babycare_id'];
 $query = " select * from tbl_babycare where babycare_id='$babycare_id'";
...
<tr>
<td>Title</td>
<td><input type="text" name="title" value="<?php echo $r1['babycare_name']?>" class="form-control" required=""/></td>
</tr>
<tr>
<td>Description</td>
<td><textarea name="des" value="" class="form-control" required=""><?php echo $r1['babycare_des']?></textarea></td>
</tr>
<tr>
<td>Image</td>
<td>
    <img src="../uploads/<?php echo $r1['babycare_img']?>" width="80" height="80"/>
...
<td></td>
<td>
    <input type="hidden" name="old" value="<?php echo $r1['babycare_img']?>"/>
...
if($file_name!='')
{
    ...
    $pic=$old;
             ...
             if( mysqli_query($con,"update tbl_babycare set babycare_name='$title',babycare_des='$des',babycare_img='$pic' where babycare_id='$babycare_id'"))
                {
                    echo '<p style="color:green;font-size:15px;font-weight:bold;
                    margin-left:100px">Details Saved Successfully.</p>';
                    echo '<script>window.location.replace("postbabycare.php")</script>';
                  }else{
